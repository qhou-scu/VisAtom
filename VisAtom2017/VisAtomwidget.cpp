    ...
    : QGLWidget(parent)
{
	...
	m_pCliper = new QList<VisAtomCliper*>[VisAtomCliper::CLIPPING_STYLE_NUM];
	for(i=0; i<VisAtomCliper::CLIPPING_STYLE_NUM; i++)
	{
		VisAtomCliper*pCliper = new VisAtomCliper();
		pCliper->AttachScene(m_Scene);
		pCliper->SetStyle(i);
		if (i == VisAtomCliper::CLIPPING_STYLE_ANYDIR) pCliper->EnableClip();
		m_pCliper[i].append(pCliper);
	}
	m_curCliperStyle = 0;
	m_curCliper      = m_pCliper[m_curCliperStyle][0];
	m_Sample->SetCliper(m_pCliper[m_curCliperStyle]);
...
{  
	...
	for(i=0; i<VisAtomCliper::CLIPPING_STYLE_NUM; i++) 
	{
		nc = this->m_pCliper[i].size();
		for (j = 0; j < nc; j++)
		{
			VisAtomCliper*pCliper = this->m_pCliper[i][j];
			delete pCliper;
		}
		this->m_pCliper[i].clear();
	}
...
void VisAtomWidget::hideEvent(QHideEvent *event)
{
	if (this->m_pickedAtomInfo)this->m_pickedAtomInfo->hide();
	return;
}
...
void VisAtomWidget::paintGL()
{
	...
	for (i = 0; i < this->m_pCliper[m_curCliperStyle].size(); i++)
	{
		if (this->m_pCliper[m_curCliperStyle][i]->IsEnabledClip())
			this->m_pCliper[m_curCliperStyle][i]->PaintEdge();
	}
	...
	//if there are selected atom, draw them 
	this->m_Sample->HighlightFoundAtoms();
	this->m_Sample->HighlightSelectedAtoms();
	...
	{
		if(this->m_pCliper[m_curCliperStyle][i]->IsEnabledClip())
			               this->m_pCliper[m_curCliperStyle][i]->PaintFace();
	}
	this->m_Scene->Paint();
...
void VisAtomWidget::mousePressEvent(QMouseEvent *event)
{
	  ...
	  {
		  if (!this->m_pickedAtomInfo->isVisible())
		  {
			  //to pick the atom 
			  this->m_pickedAtomInfo->StartPickAtomInfo(event->globalX(), event->globalY());
...
VisAtomCliper* VisAtomWidget::NewCliper()
{
	if (this->m_curCliper->GetStyle() == VisAtomCliper::CLIPPING_STYLE_ANYDIR)
	{
		VisAtomCliper*pCliper = new VisAtomCliper();
		pCliper->SetStyle(VisAtomCliper::CLIPPING_STYLE_ANYDIR);
		pCliper->AttachScene(this->m_Scene);
		this->m_pCliper[VisAtomCliper::CLIPPING_STYLE_ANYDIR].append(pCliper);
		this->m_curCliper = pCliper;
		this->m_Sample->AddCliper(pCliper);
		return pCliper;
	}
	else
	{
...
VisAtomCliper* VisAtomWidget::SetCurrentClipper(int index)
{
	switch (m_curCliper->GetStyle())
	{
	...
	case VisAtomCliper::CLIPPING_STYLE_ANYDIR:
		...
		m_curCliper = this->m_pCliper[VisAtomCliper::CLIPPING_STYLE_ANYDIR][index];
	}
	return m_curCliper;
};

void VisAtomWidget::SetCurrentCliperParameters(VisAtomCliper*sor)
{
	int style = sor->GetStyle();
	//note: style is the index of m_pCliper in VisAtomWidget class
	this->m_curCliper->SetParameters(sor);
	this->m_Sample->ResetVisibility();
	this->updateGL();
}

void VisAtomWidget::SetCurrentClipperStyle(int curstyle, bool redraw)
{
	if (curstyle == this->m_curCliperStyle)
	{
		return;
	}
    this->m_curCliperStyle = curstyle;
	this->m_curCliper = this->m_pCliper[curstyle][0];
	this->m_Sample->SetCliper(this->m_pCliper[curstyle]);
	if(redraw) this->updateGL();
}

void VisAtomWidget::OnClipXBack()
{
	VisAtomCliper*p = this->CurrentClipper();
	double pos[3], step[3];
	p->GetClipPos(pos);
	...
	p->SetClipPos(pos);
	this->m_Sample->SetCliper(this->m_pCliper[this->m_curCliperStyle]);
	this->updateGL();
    return;
}
...
void VisAtomWidget::OnActionFindatom()
{
	this->m_pickedAtomInfo->hide();
}