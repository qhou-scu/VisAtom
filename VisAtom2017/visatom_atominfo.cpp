...
void VisAtom_AtomInfo::ShowAtomInfo(int posx, int posy, VisAtomSample*pSample, int atomid)
{
		 ...
		 if(atomid >0)
		 {
		   ...
		   itemx->setText(0, tr("POS-x:"));
		   col = pDataSheet->GetPosXCol();
		   snum.setNum(pDataSheet->GetData(col)[atomid],'e',4);
		   itemx->setText(1, snum);
		   this->ui.treeWidget->addTopLevelItem(itemx);
		   QTreeWidgetItem*itemy = new QTreeWidgetItem();
		   itemy->setText(0, tr("POS-y:"));
		   col = pDataSheet->GetPosYCol();
		   snum.setNum(pDataSheet->GetData(col)[atomid],'e',4);
		   itemy->setText(1, snum);
		   this->ui.treeWidget->addTopLevelItem(itemy);
		   //snum.append(tr(", "));
		   QTreeWidgetItem*itemz = new QTreeWidgetItem();
		   itemz->setText(0, tr("POS-z:"));
...
void VisAtom_AtomInfo::StartPickAtomInfo(int posx, int posy)
{
     this->ui.treeWidget->clear();
